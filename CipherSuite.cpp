...
void CipherSuite::encryptAES(byte key[], const std::string &input_path) {
    ...
    int ret = wc_AesGcmEncrypt(&this->aes, this->cipher, plain, this->cipher_size, this->iv, sizeof(this->iv),
                               this->authTag,
...
void CipherSuite::decryptAES(byte key[], byte *cipher, size_t ciphSzs, byte *authTag, size_t authTagSz, byte *authIn,
                             size_t authInSz) {
    wc_AesInit(&this->aes, NULL, 0);
    wc_AesGcmSetKey(&this->aes, key, 32);
