...
void encrypt_block(CipherSuite& cipherSuite, byte* key, byte* buffer,
                   byte* cipher_block, byte* iv, byte* authTag,
  ...
  int ret = wc_AesGcmEncrypt(&aes, cipher_block, buffer, read_size, iv, IV_SIZE,
                             authTag, AUTH_TAG_SIZE, cipherSuite.authIn,
                             sizeof(cipherSuite.authIn));
  if (ret != 0) {
...
void decrypt_block(CipherSuite& cipherSuite, byte* key, byte* buffer,
  ...
  int ret = wc_AesGcmDecrypt(&aes, decrypted_block, buffer, read_size, iv,
                             IV_SIZE, authTag, AUTH_TAG_SIZE,
...
void CipherSuite::encryptAES(byte key[], const std::string& input_path,
  ...
  for (int i = 0; i < THREAD_POOL_SIZE; i++) {
    size_t current_block_size =
        (i == THREAD_POOL_SIZE - 1) ? last_block_size : block_size;
    ...
    threads.emplace_back(encrypt_block, std::ref(*this), key, buffer.data(),
                         cipher_block.data(), iv.data(), authTag.data(),
                         std::ref(read_size), std::ref(i), std::ref(mtx),
                         std::ref(cv), std::ref(active_threads));
    ...
    outfile.write(reinterpret_cast<const char*>(authTag.data()),
                  authTag.size());
    outfile.write(reinterpret_cast<const char*>(cipher_block.data()),
                  read_size);
  }
...
void CipherSuite::decryptAES(byte key[], const std::string& input_path,
                             const std::string& output_path) {
  wc_AesInit(&this->aes, NULL, 0);
  wc_AesGcmSetKey(&this->aes, key, 32);
  ...
  unsigned long file_size = std::filesystem::file_size(p);
  size_t block_size =
      (file_size - THREAD_POOL_SIZE * (IV_SIZE + AUTH_TAG_SIZE)) /
      THREAD_POOL_SIZE;

  size_t last_block_size =
      (file_size - THREAD_POOL_SIZE * (IV_SIZE + AUTH_TAG_SIZE)) -
      block_size * (THREAD_POOL_SIZE - 1);
  ...
  for (int i = 0; i < THREAD_POOL_SIZE; i++) {
    ...
    threads.emplace_back(decrypt_block, std::ref(*this), key, buffer.data(),
                         decrypted_block.data(), iv.data(), authTag.data(),
                         std::ref(read_size), std::ref(i), std::ref(mtx),
                         std::ref(cv), std::ref(active_threads));
    ...
    outfile.write(reinterpret_cast<const char*>(decrypted_block.data()),
                  read_size);
  }
